...
#define ELEC_TO_LIFETIME(elec) ((elec) > lifetime_low_limit_[fan_gears_] ? LIFETIME_NORMAL : LIFETIME_LOW)

static uint8_t fan_power_table[4] = {0, 25, 50, 100};

void PurifierModule::Init() {
  AppParmInfo *parm = &registryInstance.cfg_;
  registryInstance.SetConnectTimeout(CONNECT_TIMEOUT_MS);
  InitFan();
  filter_element_.Init(FILTER_ELEMENT_PIN, INPUT_PULLUP);
  InitAdcCheck();
  InitLight();
  LoadLifetime();
...
void PurifierModule::InitFan() {
  fan_power_.Init(PURIFIER_FAN_POWER, 0, OUTPUT);
  fan_power_.Out(0);
...
void PurifierModule::LoadLifetime() {
  cur_lifetime = registryInstance.cfg_.purifier_lifetime;
  cur_lifetime = (cur_lifetime > LIFETIME_NORMAL) ? LIFETIME_NORMAL :cur_lifetime;
}

void PurifierModule::SaveLifetime() {
  registryInstance.cfg_.purifier_lifetime = cur_lifetime;
...
void PurifierModule::CheckLifetime() {
  ...
  if (times == 50) {
    ...
    for (i = 0; i < LIFETIME_STABLE_TIMES - 1; i++) {
      last_lifetime[i] = last_lifetime[i + 1];
    }
    last_lifetime[i] = lifetime;

    for (i = 0; i < LIFETIME_STABLE_TIMES; i++) {
      if (last_lifetime[0] != last_lifetime[i] || last_lifetime[i] > LIFETIME_NORMAL)
        break;
    }
    if ((i == LIFETIME_STABLE_TIMES) && (last_lifetime[0] != cur_lifetime)) {
        cur_lifetime = last_lifetime[0];
        ReportLifetime();
        SaveLifetime();
    }
  }
...
uint8_t PurifierModule::WorkExceptionCheck() {
  ...
  if (filter_element_.Read() == 1) {
    ...
    if (!err_ && IsFanStable()) {
      ...
      // if (temp >= open_lid_limit_[fan_gears_]) {
...
void PurifierModule::WorkProcess() {
  if (!WorkExceptionCheck()) {
    LightCtrl(PURIFIER_LIGHT_STA_E(LT_LIFETIME_LOW + cur_lifetime));
    CheckLifetime();
    FanOut(fan_gears_);
  } else {
...
void PurifierModule::PeripheralLoopCtrl() {
  fan_.SpeedOutCtrl();
  light_.Loop();
  filter_element_.CheckStatusLoop();
  if (last_err_ != err_) {
    last_err_ = err_;
    ReportErrStatus();
  }
